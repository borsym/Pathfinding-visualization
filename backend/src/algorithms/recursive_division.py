...
class RecursiveDivison:
    ...
    def divide(self, grid, x, y, width, height, orientation):
        if width < 2 or height < 2: # first checks to see if the maze has reached the target
            return
        horizontal = orientation == self.HORIZONTAL

        truewidth = math.floor((width-1)/2); # le vagy fel kerekit?
        trueheight = math.floor((height-1)/2); 
        # where will the wall be drawn from?
        wx = x + (0 if horizontal else (0 if truewidth == 0 else 2 * randrange(truewidth) + 1)) # lehet + 1 minden range-ben
        wy = y + ( (2 * randrange(trueheight) + 1 if trueheight > 0 else 0) if horizontal else 0)
        # print(f'wx={wx}, wy={wy}')
        # where will the passage through the wall exist?
        px = wx + ((2 * randrange(truewidth) if truewidth > 0 else 0) if horizontal else 0)
        py = wy + (0 if horizontal else (2 * randrange(trueheight) if trueheight > 0 else 0))
        # print(f'px={px}, py={py}')
...
# width = table.get_column_size()
# height = table.get_row_size()
# recdiv = RecursiveDivison(table,0,0,width,height)
# a = recdiv.start_divide()
# print(a)
# f = open("test_algorithm.txt", "w")
# count = 0
